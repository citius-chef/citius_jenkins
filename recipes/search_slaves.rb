slave = node['dcar_jenkins']['slave_tag']
######################################################
query = ' tags:"' + slave + '"'
...
slave_nodes.each do |each_slave|
  node.override['dcar_jenkins']['slave_name'] = each_slave['name'] + '-' + each_slave['platform_family']
  slavename = node['dcar_jenkins']['slave_name']
  log(slavename + ' is the slave')

  if each_slave['platform_family'] == 'windows'
    if each_slave['label'] == 'WinBDD'
      executor = 1
    elsif each_slave['label'] == 'WinBuild'
      executor = 3
    end

    slave_ip = each_slave['ip']
    label_name = each_slave['label']
    slave = node['dcar_jenkins']['windows_slaves']

    jenkins_slave slavename do
      description      slave['description'] + each_slave['ip'] if slave['description']
    ...
    slave = node['dcar_jenkins']['linux_slaves']
